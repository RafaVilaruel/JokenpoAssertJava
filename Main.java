...
//  Rafael Vilaruel
//  https://github.com/rafavilaruel (versão deste programa em inglês)
//  www.linkedin.com/in/rafaelvilaruel (para contato e outros projetos)
...
public class Main{		
	...
	int userScore = 0;
	    ...
	    //  Assert Tests, used to build the application. Test first, code later!!		
				...
				if (checkHands3(userChosen)) {
					System.out.println("Você estragou tudo...Escolha uma das opções!!!");
	...
	try {TimeUnit.SECONDS.sleep(3);
				...
				showResults(userChosen,pcChosen,userScore,pcScore);
...
public static String sortResults(int move) {
	...
	public static void pcDialogue(int pcChosen) {
		...
		System.out.println("Vamos lá! Jo..Ken..PÔ!!!");
		try {TimeUnit.SECONDS.sleep(3);
		} catch (InterruptedException e2) {e2.printStackTrace();}
...
public static void showResults(int userChosen, int pcChosen,int userScore,int pcScore) {
		...
		// Portuguese sucks!!! My english version didnt need this patch	
		// Lingua portuguesa é osso!!! Minha versão em inglês não teve este problema.
		if (p1.equals("sua Papel")) {p1 = "seu Papel";}
		if (p2.equals("minha Papel")) {p2 = "meu Papel";}
		...
		System.out.println();
		System.out.println("╔═══════════════════╗");
		System.out.println("║      Você: "+userScore+"      ║");
		System.out.println("║      Eu: "+pcScore+"        ║");
		System.out.println("╚═══════════════════╝");
		...
		int userChosen3 = scanner2.nextInt();
		
		if(userChosen3 == 1){
		  
		  int userScore2 = userScore;
		int pcScore2 = pcScore;
		  
		  userInterface();
		  
		  
		  int userChosen2 = scanner2.nextInt()*-1;;
				int pcChosen2 = (int) (Math.random()*-3);
				//scanner2.close();
		  
		  if (checkHands3(userChosen2)) {
					System.out.println("Você estragou tudo...Escolha uma das opções!!!");
		System.out.println("Desligando o jogo.");			
	try {TimeUnit.SECONDS.sleep(3);
		} catch (InterruptedException e2) {e2.printStackTrace();}				
					System.exit(0);}			
				if (checkHands(userChosen2,pcChosen2)) {
					userChosen2 = newPaper;}			
				if (checkHands2(userChosen,pcChosen)) {
					pcChosen2 = newPaper;}
										
				//	PC dialogue
				//  Diálogo da Máquina
				pcDialogue(pcChosen2);
			
				//	sort and show game results to the user
				//  ajeita e mostra os resultados do jogo para o usuário
				showResults(userChosen2,pcChosen2,userScore2,pcScore2);
